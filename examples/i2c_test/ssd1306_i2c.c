...
Modified by shirogane to work with linux i2c apis
*********************************************************************/
...
// Init SSD1306
void ssd1306_begin(struct I2cDevice *dev)
{
	// I2C Init
...
void ssd1306_display(void)
{
	...
	for (i = 0; i < (SSD1306_LCDWIDTH * SSD1306_LCDHEIGHT / 8); i++) {
		i2c_write_reg(g_dev, 0x40, buffer[i]); 
...
inline static void swap_values(int *a, int *b)
{
	int t = *a;
	*a = *b;
	*b = t;
