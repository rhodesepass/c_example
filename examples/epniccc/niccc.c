...
// printf("start = %p\n", &incbin_scene_start);
// printf("end = %p\n", &incbin_scene_end);
// printf("size = %zu\n", (char*)&incbin_scene_end - (char*)&incbin_scene_start);
// printf("first byte = 0x%02hhx\n", incbin_scene_start[0]);
...
static bool build_frame_index_once(void) {
    ...
    g_stream = (const uint8_t*)(const void*)incbin_scene_start;
    g_stream_size = (size_t)((const uint8_t*)(const void*)incbin_scene_end - (const uint8_t*)(const void*)incbin_scene_start);
...
static void blit_scaledfb_ccw_to_vaddr_bottom(uint32_t* vaddr) {
    if (!vaddr) return;
    const int dst_y0 = DRAW_HEIGHT - NICCC_ROT_H; // 640 - 460 = 180
    if (dst_y0 < 0) return;

    // source: 460x360, dest: 360x460 (CCW)
    for (int sy = 0; sy < NICCC_SCALED_H; sy++) {
        const uint32_t* srow = g_scaled_fb + sy * NICCC_SCALED_W;
        const int dx = sy; // 0..359
        for (int sx = 0; sx < NICCC_SCALED_W; sx++) {
            const uint32_t spx = srow[sx];
            const int dy = (NICCC_SCALED_W - 1) - sx; // 0..459
            const int dst_x = dx;
            const int dst_y = dst_y0 + dy;
            vaddr[dst_y * DRAW_WIDTH + dst_x] = spx;
        }
    }
}
...
void niccc_draw_frame(uint32_t* vaddr,int frame_idx){
    ...
    blit_scaledfb_ccw_to_vaddr_bottom(vaddr);
