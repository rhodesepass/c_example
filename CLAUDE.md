...
- **绘图库 (`lib/fbdraw.h`, `lib/fbdrawttf.h`)**:
  ...
  - 当用户要求显示中文时，使用fbdraw_ttf_font_t进行字体渲染，否则使用RREFont进行渲染。
    ...
    - 使用RREFont渲染字体，背景颜色请设置为不透明，如setBg(0xFF000000)
...
{
    "version": 1, //必选，版本号
    "name": "应用名称", //可选，没有就用文件夹名称
    "uuid":"shell调用uuidgen生成一个随机的", //必选，uuid
    "description": "应用描述", //可选，不存在则为“无描述”
    "icon": "icon.png", // 程序图标，可以为空。如果为空，则使用默认图标。
    "executable": { //必选，可执行文件名
        "file": "可执行文件名"
    },
    "type": "fg_ext", //必选，bg：后台运行。fg：前台运行，fg_ext：前台运行，且只可以通过拓展名启动，拒绝从菜单直接启动。
    "extensions": [".txt", ".log"], // 可选，关联的文件后缀名。
    "screens":["360x640","480x854","720x1280"] // 必选，此应用支持的屏幕分辨率。
}
```
如果用户没有特殊说明，则只需要兼容360x640分辨率。
